...
# Format: build.sh file1 file2 ... fileN
...
echo "$*" "${flags}" | xargs g++ -o ./quadratic >./.tmp_error_log.txt 2>&1

if [[ "$?" -eq 0 ]]; then
  echo Programm successfully builded to ./quadratic
else
  echo An error occurred. You can read error log in ./error_log.txt
  cat ./.tmp_error_log.txt > ./error_log.txt
fi

rm -f ./.tmp_error_log.txt
